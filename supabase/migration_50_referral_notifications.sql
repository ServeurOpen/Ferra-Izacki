-- ============================================================
-- Izacki — Migration 50 : rendre le versement de parrainage VISIBLE
-- (06/09/2026, demande explicite : "comment on pourrait faire pour
-- mettre une sécurité et être sûr que si la récompense a été
-- distribuée"). Jusqu'ici, même quand tout fonctionne, ni le parrain ni
-- le filleul ne voyaient de confirmation explicite — juste un solde qui
-- change en silence, difficile à distinguer visuellement de "rien ne
-- s'est passé".
--
-- Filleul : reçoit maintenant un toast dédié au moment même de sa
-- récompense quotidienne (claim_daily_reward renvoie l'info en plus).
-- Parrain : n'est pas en train d'utiliser l'app à ce moment précis (c'est
-- le FILLEUL qui appelle la fonction) — réutilise credit_gift_notifications
-- (déjà affiché au prochain démarrage/moment où le parrain est en ligne,
-- voir checkCreditGifts côté Launcher), avec une colonne "reason" ajoutée
-- pour distinguer "parrainage" d'un vrai don du créateur (texte différent
-- affiché côté client).
-- À exécuter UNE FOIS dans Supabase, APRÈS migration_49.
-- ============================================================

alter table public.credit_gift_notifications add column if not exists reason text;
...
declare
  ...
  v_referred_bonus int := 10;       -- versé au FILLEUL
...
begin
  ...
  if v_new_day = 3 and v_referrer is not null then
    ...
    if not v_already_rewarded then
      ...
      if (v_device is null or v_device_count < v_referral_id_cap)
        ...
        -- Notification visible pour le parrain (06/09/2026, nouveau) —
        -- il n'est pas en train d'utiliser l'app à cet instant précis.
        insert into public.credit_gift_notifications (target_user_id, amount, reason)
          values (v_referrer, v_referral_amount, 'parrainage');
  ...
  return jsonb_build_object(
    'claimed', true, 'day', v_new_day, 'amount', v_amount, 'newBalance', v_new_balance,
    'referralBonusPaid', v_referral_paid, 'referredBonusAmount', case when v_referral_paid then v_referred_bonus else null end
  );
end;
$$;
